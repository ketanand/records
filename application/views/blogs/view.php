...
var blogId = <?php echo $blog['id'] ?>; //TODO: echo blog id here
function reveal()
{
...
function login(){
	FB.login(function(response){
		if (response.authResponse) {
			...
			var html = "<ul class='account-dd'><li class='has-dropdown login'><i class='icon icon-user' title='Login'></i> <i class='icon icon-angle-down'></i><div class='comments-dropdown'><ul class='actions'><li><a href='#' class='logout' rel='logout' data-event-category='topbar' data-event-action='click' data-event-label='Topbar: Logout'>Log&nbsp;Out</a></li> </ul></div></li></ul>";
			jQuery('button.account-login').replaceWith(html);
			unreveal();
			loggedIn = true;
	        } else {
...
function submitComment(){
	...
	if (!loggedIn){
		...
		var data = jQuery('#comment-form').serialize()+'&id='+blogId;
		jQuery.ajax({
			type:"POST",
			dataType : 'json',
			data : data,
			...
			success: function(response){
				    if(response.success){
					var html = "<li class='comment-item'><div id='comment-" + response.data.id + "' class='container-in indent-0' ><article data-comment-id='" + response.data.id + "' data-thread-key='fastcoexist:3021967' data-depth='0' data-is-hidden='' class='comment clearfix'><header><span class='author name' data-author-provider='facebook' data-author-name='" + response.data.comment_author + "' data-author-screen-name='" + response.data.comment_author_screen_name + "'>" + response.data.comment_author + "</span><span class='parent-author'>" + response.data.comment_parent_author + "</span><time class='datess' data-created-at='" + response.data.created_at + "' datetime='" + response.data.created_at + "' title='" + response.data.created_at + "' ><a href='#comment-" + response.data.id + "'>2 days ago</a></time></header><span class='text node-article'><p>" + response.data.comment + "</p></span></article></div></li>";
...
//Functions to call after window load
jQuery(window).load(function(){
	...
	jQuery.ajax({
				type:"GET",
				dataType : 'json',
				url: "http://" + window.location.host + "/blogs/getcomment/" + blogId,
				success: function(response){
				    if(response.success){
					var html = '';
					...
					html += "<div id='id-comments' class='comment-list'><div class='container-in'><ul class='comment-list' id='comment-list' >";
			...
			jQuery.ajax({
				type:"GET",
				dataType : 'json',
				url: "http://" + window.location.host + "/blogs/getrelated/" + blogId,
				success: function(response){
				    if(response.success){
					var html = '';
	...
	<section id="comments-publish">
		...
		<div id="comment-reply" class="comment-reply">
		    ...
		    <form id="comment-form" class="comment-reply-form clearfix">
		    	    <textarea id="comment-text" name="comment" onClick="openSubmit()" maxlength="1000" rows="7" cols="80" placeholder="Type your comment here." required></textarea>
			    <a href="javascript:void(0)"name="comment_reply_submit" onClick="submitComment()" class="submit">Submit</a>
		    </form>
	    </div>
	</div>
